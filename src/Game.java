...
public class Game {
	...
	private static final String TXTHELP = "<html><center><H2>Help</H2></center><center>A game for two, in which the " +
			"players first choose the color of the chips, and then take turns dropping the chips into the cells of the " +
			"vertical board.<br> The goal of the game is to place four chips of your own color horizontally, vertically, " +
			"or diagonally before the opponent in a row. There are variants of the game with a field of different sizes," +
			" with chips in the form of disks or balls.</center></html>";

	private static final String TITLE_ABOUT = "About";
	private static final String TXTABOUT = "<html><center><H2>About</H2></center><center>American Milton Bradley " +
			"patented this game in 1974 under the name Connect Four. Robert Bell described it under the name " +
			"Four balls and dated it to the reign of King Edward (1901-1910). At that time, the set for the game was " +
			"a wooden box from a bottle of cognac Remy Martin with a hinged lid, on which eight vertical guides and a " +
			"sliding bottom were placed in a row, and balls made of beech and mahogany were used as chips. Bell noted " +
			"that, judging by the workmanship, this is clearly a copy from an older set. There is evidence that the " +
			"great navigator James Cook adored her so much that he always took her with him on a voyage. " +
			"He and his companions – the naturalist Joseph Banks and the botanist Daniel Solander – could spend hours " +
			"playing it during long sea crossings, for which the sailors, not without malice, called the box with " +
	...
	public static void placeChecker(int color, int row, int col) {
		...
		checker.setBounds(27 + (75 * col), 27 + (75 * row), checkerIcon.getIconWidth(),checkerIcon.getIconHeight());
	...
	public static KeyAdapter gameKey = new KeyAdapter() {
		...
		public void keyPressed(KeyEvent e) {
			...
			if (((e.getModifiersEx() & KeyEvent.CTRL_DOWN_MASK) != 0) && (e.getKeyCode() == KeyEvent.VK_U)) {
                undo();
            } else if (((e.getModifiersEx() & KeyEvent.CTRL_DOWN_MASK) != 0) && (e.getKeyCode() == KeyEvent.VK_R)) {
				redo();
            }
		}
	};
